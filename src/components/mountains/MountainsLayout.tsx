...
export function MountainsLayout({ children }: { children: ReactNode }) {
  ...
  const pageBg = isDark
    ...
    : "linear-gradient(180deg, #0e1a0e 0%, #1c1510 10%, #f0e8d5 28%, #faf5ec 40%)";
  ...
  return (
    <div className="min-h-screen transition-colors duration-500" style={{ background: pageBg }}>
      {/* Hero - always dark, left-aligned like History/Regions */}
      <section className="relative overflow-hidden" style={{ minHeight: "36vh", display: "flex", alignItems: "flex-end", paddingBottom: "48px" }}>
        <div className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 60% at 30% 50%, rgba(44,72,34,.35), transparent 70%), radial-gradient(ellipse 50% 80% at 80% 20%, rgba(201,160,82,.06), transparent 60%)" }} />
        <svg className="pointer-events-none absolute bottom-0 left-0 right-0 h-[40%] w-full opacity-[0.08]" viewBox="0 0 1440 200" preserveAspectRatio="none">
        ...
        <div className="pointer-events-none absolute right-[5%] top-[12%] select-none" style={{ fontFamily: "var(--font-brush), serif", fontSize: "clamp(4rem,8vw,7rem)", color: "rgba(201,160,82,0.04)", writingMode: "vertical-rl", letterSpacing: "0.15em" }}>
          百座茶山
        </div>
